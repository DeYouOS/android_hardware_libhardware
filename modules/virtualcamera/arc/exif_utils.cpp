...
bool ExifUtils::Initialize(const uint8_t* buffer, uint16_t width, uint16_t height, int quality) {
    Reset();
...
bool ExifUtils::SetMaker(const std::string& maker) {
    ...
    std::unique_ptr<ExifEntry> entry =
            AddVariableLengthEntry(EXIF_IFD_0, EXIF_TAG_MAKE, EXIF_FORMAT_ASCII, entrySize,
                                   entrySize);
    if (!entry) {
...
bool ExifUtils::SetModel(const std::string& model) {
    ...
    std::unique_ptr<ExifEntry> entry =
            AddVariableLengthEntry(EXIF_IFD_0, EXIF_TAG_MODEL, EXIF_FORMAT_ASCII, entrySize,
                                   entrySize);
    if (!entry) {
...
bool ExifUtils::SetDateTime(const struct tm& t) {
    ...
    std::unique_ptr<ExifEntry> entry =
            AddVariableLengthEntry(EXIF_IFD_0, EXIF_TAG_DATE_TIME, EXIF_FORMAT_ASCII, sizeof(str),
                                   sizeof(str));
    if (!entry) {
...
bool ExifUtils::SetGpsLatitude(double latitude) {
    ...
    std::unique_ptr<ExifEntry> entry =
            AddVariableLengthEntry(EXIF_IFD_GPS, tag, EXIF_FORMAT_RATIONAL, 3,
...
bool ExifUtils::SetGpsAltitude(double altitude) {
    ...
    std::unique_ptr<ExifEntry> entry =
            AddVariableLengthEntry(EXIF_IFD_GPS, tag, EXIF_FORMAT_RATIONAL, 1,
                                   sizeof(ExifRational));
    if (!entry) {
        exif_content_remove_entry(exif_data_->ifd[EXIF_IFD_GPS], refEntry.get());
...
bool ExifUtils::SetGpsTimestamp(const struct tm& t) {
    ...
    std::unique_ptr<ExifEntry> entry =
            AddVariableLengthEntry(EXIF_IFD_GPS, dateTag, EXIF_FORMAT_ASCII, kGpsDateStampSize,
                                   kGpsDateStampSize);
    if (!entry) {
...
bool ExifUtils::SetGpsProcessingMethod(const std::string& method) {
    ...
    std::unique_ptr<ExifEntry> entry =
            AddVariableLengthEntry(EXIF_IFD_GPS, tag, EXIF_FORMAT_UNDEFINED, size, size);
    if (!entry) {
...
std::unique_ptr<ExifEntry> ExifUtils::AddVariableLengthEntry(ExifIfd ifd, ExifTag tag,
                                                             ExifFormat format, uint64_t components,
                                                             unsigned int size) {
...
bool ExifUtils::GenerateThumbnail() {
    ...
    if (!compressor_.CompressImage(scaled_buffer.data(), thumbnail_width_, thumbnail_height_,
                                   thumbnail_jpeg_quality_, NULL, 0)) {
...
bool ExifUtils::GenerateYuvThumbnail(std::vector<uint8_t>* scaled_buffer) {
    ...
    int result = libyuv::I420Scale(y_plane, yu12_width_, u_plane, yu12_width_ / 2, v_plane,
                                   yu12_width_ / 2, yu12_width_, yu12_height_, scaled_y_plane,
                                   thumbnail_width_, scaled_u_plane, thumbnail_width_ / 2,
                                   scaled_v_plane, thumbnail_width_ / 2, thumbnail_width_,
                                   thumbnail_height_, libyuv::kFilterNone);
    if (result != 0) {
