...
bool JpegCompressor::CompressImage(const void* image, int width, int height, int quality,
    ...
    LOGF(INFO) << "Compressed JPEG: " << (width * height * 12) / 8 << "[" << width << "x" << height
               << "] -> " << result_buffer_.size() << " bytes";
    return true;
}
...
bool JpegCompressor::Encode(const void* inYuv, int width, int height, int jpegQuality,
                            const void* app1Buffer, unsigned int app1Size) {
