...
class ExifUtils : virtual public android::RefBase {
public:
    ExifUtils();
    ~ExifUtils();
    ...
    bool Initialize(const uint8_t* buffer, uint16_t width, uint16_t height, int quality);
...
private:
    ...
    // two.
    std::unique_ptr<ExifEntry> AddVariableLengthEntry(ExifIfd ifd, ExifTag tag, ExifFormat format,
