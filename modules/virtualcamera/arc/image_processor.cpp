...
size_t ImageProcessor::GetConvertedSize(int fourcc, uint32_t width,
                                        uint32_t height) {
    if ((width % 2) || (height % 2)) {
        LOGF(ERROR) << "Width or height is not even (" << width << " x "
                    << height << ")";
        return 0;
    }
...
bool ImageProcessor::SupportsConversion(uint32_t from_fourcc,
    ...
    case V4L2_PIX_FMT_YUV420:
        return (
            ...
            to_fourcc == V4L2_PIX_FMT_YVU420 ||
            to_fourcc == V4L2_PIX_FMT_NV21 || to_fourcc == V4L2_PIX_FMT_RGB32 ||
            to_fourcc == V4L2_PIX_FMT_BGR32 || to_fourcc == V4L2_PIX_FMT_JPEG);
...
int ImageProcessor::ConvertFormat(const CameraMetadata &metadata,
    ...
    if (in_frame.GetFourcc() == V4L2_PIX_FMT_YUYV) {
        ...
        case V4L2_PIX_FMT_YUV420: // YU12
        {
            ...
            LOGF(ERROR) << "YUY2ToI420() for YU12 returns " << res;
            return res ? -EINVAL : 0;
        }
        default:
    ...
    if (metadata.exists(ANDROID_JPEG_THUMBNAIL_SIZE)) {
        ...
        if (thumbnail_width > 0 && thumbnail_height > 0) {
            if (!utils->SetThumbnailSize(
                    static_cast<uint16_t>(thumbnail_width),
