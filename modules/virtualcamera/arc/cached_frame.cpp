...
CachedFrame::CachedFrame()
    : source_frame_(nullptr), cropped_buffer_capacity_(0),
...
int CachedFrame::SetframeSource(const FrameBuffer *frame) {
    source_frame_ = frame;
    yu12_frame_->SetDataSize(frame->GetDataSize());
    yu12_frame_->SetFourcc(frame->GetFourcc());
    yu12_frame_->SetWidth(frame->GetWidth());
    yu12_frame_->SetHeight(frame->GetHeight());
    memcpy(yu12_frame_->GetData(), frame->GetData(), frame->GetBufferSize());
    return 0;
}
...
int CachedFrame::CropRotateScale(int rotate_degree) {
    ...
    if (yu12_frame_->GetHeight() % 2 != 0 || yu12_frame_->GetWidth() % 2 != 0) {
        LOGF(ERROR) << "yu12_frame_ has odd dimension: "
                    << yu12_frame_->GetWidth() << "x"
                    << yu12_frame_->GetHeight();
        return -EINVAL;
    }
    ...
    //
    //   Original frame                  Cropped frame              Rotated
    //   frame
    // --------------------               --------
    // |     |      |     |               |      | --------------- |     | | |
    // |      |                 |             | |     |      |     |   =======>>
