...
#ifndef HAL_CACHED_FRAME_H_
#define HAL_CACHED_FRAME_H_
...
class CachedFrame : virtual public android::RefBase {
  public:
    CachedFrame();
    ~CachedFrame();

    // SetSource() doesn't take ownership of |frame|. The caller can only
    // release |frame| after calling UnsetSource(). SetSource() immediately
    // converts incoming frame into YU12. Return non-zero values if it
    // encounters errors. If |rotate_degree| is 90 or 270, |frame| will be
    // cropped, rotated by the specified amount and scaled. If |rotate_degree|
    // is -1, |frame| will not be cropped, rotated, and scaled. This function
    // will return an error if |rotate_degree| is not -1, 90, or 270.
    int SetSource(const FrameBuffer *frame, int rotate_degree);
    int SetframeSource(const FrameBuffer *frame);
    void UnsetSource();
  ...
  private:
    int ConvertToYU12();
    // When we have a landscape mounted camera and the current camera activity
    // is portrait, the frames shown in the activity would be stretched.
    // Therefore, we want to simulate a native portrait camera. That's why we
    // want to crop, rotate |rotate_degree| clockwise and scale the frame. HAL
    // would not change CameraInfo.orientation. Instead, framework would fake
    // the CameraInfo.orientation. Framework would then tell HAL how much the
    // frame needs to rotate clockwise by |rotate_degree|.
