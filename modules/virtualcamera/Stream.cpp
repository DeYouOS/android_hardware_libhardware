...
Stream::Stream(int id, camera3_stream_t* s) : mReuse(false), mId(id), mStream(s) {}
...
bool Stream::isValidReuseStream(int id, camera3_stream_t* s) {
    if (id != mId) {
        LOGF(ERROR) << " " << mId << " Invalid camera id for reuse. Got " << id << " expect";
        return false;
    }
    if (s != mStream) {
        LOGF(ERROR) << " " << mId << " Invalid stream handle for reuse. Got " << s << " expect "
                    << mStream;
        return false;
    }
    if (s->stream_type != mStream->stream_type) {
        LOGF(ERROR) << " " << mId << " Mismatched type in reused stream. Got "
                    << typeToString(s->stream_type) << "(" << s->stream_type
                    << ") "
                       "expect "
    ...
    if (s->format != mStream->format) {
        LOGF(ERROR) << " " << mId << " Mismatched format in reused stream. Got "
                    << formatToString(s->format) << "(" << s->format << ") expect "
                    << formatToString(mStream->format) << "(" << mStream->format << ")";
        return false;
    }
    if (s->width != mStream->width) {
        LOGF(ERROR) << " " << mId << " Mismatched width in reused stream. Got " << s->width
                    << " expect " << mStream->width;
        return false;
    }
    if (s->height != mStream->height) {
        LOGF(ERROR) << " " << mId << " Mismatched height in reused stream. Got " << s->height
                    << " expect " << mStream->height;
        return false;
    }
    return true;
}
